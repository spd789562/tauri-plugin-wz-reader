use super::get_node_without_parse;
use crate::{Error, Result};

use image::DynamicImage;
use wz_reader::{node, property::string, WzNodeArc, WzNodeCast};
...
pub fn resolve_png(node: &WzNodeArc, root: Option<&WzNodeArc>) -> Result<DynamicImage> {
    ...
    if let Some(png) = node_read.try_as_png() {
        let inlink_target = node_read
            ...
            .and_then(|link| node::resolve_inlink(&link, node));
        ...
        let outlink_target = node_read
            ...
            .and_then(|link| {
                if let Some(root) = root {
                    get_node_without_parse(root, &link)
                } else {
                    node::resolve_outlink(&link, node, true)
                }
            });
...
pub fn resolve_png_form_root(root: &WzNodeArc, path: &str) -> Result<DynamicImage> {
    let target =
        get_node_without_parse::get_node_without_parse(root, path).ok_or(Error::NodeNotFound)?;
